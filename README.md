...
This repository is an example layout intended to be copied or symlinked into `~/.config/codex`.
...
# Clone this repository to ~/.config/codex
git clone https://github.com/feiskyer/codex-settings.git ~/.config/codex

# Or symlink if you prefer to keep it elsewhere
ln -s /path/to/codex-settings ~/.config/codex
```
...
2. Start LiteLLM proxy:
   ...
   ```bash
   litellm --config ~/.config/codex/litellm_config.yaml
...
- [config.toml](config.toml): Default configuration using LiteLLM gateway
  - Model: `gpt-5.2` via `model_provider = "github"` (Copilot proxy on `http://localhost:4000`)
...
1. Create a new `.md` file in `~/.config/codex/prompts/`
...
~/.config/codex/skills/autonomous-skill/scripts/run-session.sh "Build a REST API for todo app"

# Continue an existing task
~/.config/codex/skills/autonomous-skill/scripts/run-session.sh --task-name build-rest-api-todo --continue

# List all tasks
~/.config/codex/skills/autonomous-skill/scripts/run-session.sh --list
```
